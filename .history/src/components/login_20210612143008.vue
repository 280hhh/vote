  ...
  methods:{
    ...
    login(){
      ...
      this.$refs.loginFormRef.validate(valid=>{
        if(!valid){
          Vue.prototype.$message({showClose:true,message:'请输入正确的账号密码',type:'error'})
          return;
        }
        flag=1;
      })
      if(flag){
        for(var item in this.userGroup){
          ...
          Vue.prototype.$message({showClose: true,message: '登陆成功',type: 'success'});
          return ;
        }
      }
      }
      Vue.prototype.$message({showClose: true,message: '账号或密码错误！！！请输入正确的账号密码',type: 'error'});
    },
    // 点击注册
    register(){
