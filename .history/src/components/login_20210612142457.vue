  ...
  methods:{
    ...
    login(){
      ...
      this.$refs.loginFormRef.validate(valid=>{
        if(!valid){
          _this.$message({showClose:true,message:'请输入正确的账号密码',type:'error'})
          return;
        }
        flag=1;
      })
      if(flag){
        for(var item in _this.userGroup){
          if(_this.loginform.username==_this.userGroup[item].username&&_this.loginform.password==_this.userGroup[item].password){
          this.$router.push({ path:'/home' });
          _this.$message({showClose: true,message: '登陆成功',type: 'success'});
          return ;
        }
      }
      }
      _this.$message({showClose: true,message: '账号或密码错误！！！请输入正确的账号密码',type: 'error'});
    },
    // 点击注册
    register(){
