  ...
  methods:{
    onSubmit(){
      ...
      let flag=0;//若预验证成功则返回1，若失败返回0
            _this.$refs.registerFormRef.validate( valid =>{
                if(!valid) {
                    _this.$message({showClose: true,message: '请输入正确的信息',type: 'error'
                    });
                    return false;
                }
                flag=1;//预验证成功！
            ...
            if(flag){
                ...
                if(localStorage.userGroup != null){
                   _this.userGroup.push(_this.registerForm);
                   localStorage.userGroup=JSON.stringify(_this.userGroup)
            
                }else{//如果之前没有任何人注册过的话
                _this.userGroup.push(_this.registerForm);
                _this.userGroup.push(_this.superUser);
                localStorage.userGroup=JSON.stringify(_this.userGroup);
                }
                _this.$router.push("/login");
                _this.$message({showClose: true,message: '注册成功！',type: 'success'});
            }else{
                _this.$message({showClose: true,message: '注册失败！请输入按要求填写注册信息！',type: 'error'});
