...
export default {
  ...
  mounted(){
        ...
        that.loginUser=JSON.parse(localStorage.userGroup);
        //更新用户组和登陆人的票数和日期
        this.oldMessage();
        for(item in that.userGroup){
            if(this.userGroup[item].username==that.loginUser.username){
            }
        }
            localStorage.loginUser=JSON.stringify(that.userGroup[item]);
            localStorage.userGroup=JSON.stringify(that.userGroup);
        }console.log(that.loginUser.username)
  }
}
</script>
